...
def test_list_categories(api_client, category):
    url = reverse("businesses:businesscategory-list")
...
def test_retrieve_category_by_slug(api_client, category):
    url = reverse("businesses:businesscategory-detail", kwargs={"slug": category.slug})
    res = api_client.get(url)
    assert res.status_code == 200
...
def test_search_category(api_client, category):
    url = reverse("businesses:businesscategory-list") + "?search=salon"
    res = api_client.get(url)
    assert res.status_code == 200
...
def test_ordering_category(api_client):
    ...
    url = reverse("businesses:businesscategory-list") + "?ordering=-name"
    res = api_client.get(url)
    assert res.status_code == 200
