# syllabus/management/commands/seed_hisabati_2023.py
...
class Command(BaseCommand):
    help = "Seed SpecificLearningActivity data for Hisabati 2023 from CSV"
    
    def add_arguments(self, parser):
        parser.add_argument(
            '--csv-file',
            type=str,
            default='syllabus/csv/hisabati_2023.csv',
            help='Path to CSV file (default: syllabus/csv/hisabati_2023.csv)',
        )

    def parse_subject_version_code(self, code: str):
        """
        Parse subject version code (e.g., 2023-JSS001-DRS III)
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    try:
                        ...
                        subject, sub_created = Subject.objects.get_or_create(
                            code=subject_code,
                            defaults={'name': f'Hisabati ({subject_code})'}
                        )
                        if sub_created:
                        ...
                        class_level, cl_created = ClassLevel.objects.get_or_create(
                            ...
                            defaults={'description': f'Darasa la {class_level_name}'}
                        )
                        if cl_created:
