# jamiiwallet/views/transfer_views.py
...
from jamiiwallet.serializers.transfer_serializer import TransferSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if serializer.is_valid(raise_exception=True):
                ...
                logger.info(f'Transfer created with id {transfer.id} for user {request.user.id}')
            ...
            logger.warning(f'Validation error in transfer by user {request.user.id}: {e}')
            return Response({'errors': e.detail}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error(f'Unexpected error creating transfer for user {request.user.id}: {e}', exc_info=True)
